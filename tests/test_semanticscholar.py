...
class SemanticScholarTest(unittest.TestCase):
    ...
    def test_dataset_diff(self) -> None:
        ...
        for i, diff in enumerate(dataset_diff.diffs):
            ...
            self.assertEqual(
                len(diff.update_files), len(data["diffs"][i]["update_files"])
            )
            self.assertEqual(
                len(diff.delete_files), len(data["diffs"][i]["delete_files"])
            )
    ...
    def test_paper(self) -> None:
        ...
        self.assertEqual(
            item.influentialCitationCount, data["influentialCitationCount"]
        )
        ...
        self.assertEqual(
            item.publicationDate, datetime.strptime(data["publicationDate"], "%Y-%m-%d")
        )
    ...
    def test_get_paper(self):
        ...
        self.assertEqual(data.title, "Computing Machinery and Intelligence")
        self.assertEqual(data.raw_data["title"], "Computing Machinery and Intelligence")
    ...
    def test_get_papers(self):
        ...
        for item in data:
            with self.subTest(subtest=item.paperId):
                self.assertIn("E. Duflo", [author.name for author in item.authors])
    ...
    def test_get_paper_citations(self):
        data = self.sch.get_paper_citations("CorpusID:14514057", fields=["title"])
        self.assertEqual(data.offset, 0)
    ...
    def test_timeout(self):
        ...
        self.assertRaises(
            TimeoutException, self.sch.get_paper, "10.1093/mind/lix.236.433"
        )
    ...
    def test_get_authors(self):
        ...
        self.assertCountEqual([item.name for item in data], list_of_author_names)
    ...
    def test_get_author_papers(self):
        ...
        self.assertEqual(
            data[0].title, "SARS-CoV-2 hijacks p38β/MAPK11 to promote virus replication"
        )

    @test_vcr.use_cassette
    def test_not_found(self):
        methods = [[self.sch.get_paper, "title"], [self.sch.get_author, "name"]]
    ...
    def test_search_paper_fields_of_study(self):
        data = self.sch.search_paper(
            "turing", fields_of_study=["Mathematics"], fields=["s2FieldsOfStudy"]
        )
    ...
    def test_search_paper_year_range(self):
        data = self.sch.search_paper("turing", year="1936-1937", fields=["year"])
        self.assertTrue(all([1936 <= item.year <= 1937 for item in data]))

    @test_vcr.use_cassette
    def test_search_paper_publication_types(self):
        data = self.sch.search_paper(
            "turing", publication_types=["JournalArticle"], fields=["publicationTypes"]
        )
        ...
        self.assertTrue(
            "Book" in data[0].publicationTypes
            or "Conference" in data[0].publicationTypes
        )

    @test_vcr.use_cassette
    def test_search_paper_venue(self):
        data = self.sch.search_paper("turing", venue=["ArXiv"], fields=["venue"])
    ...
    def test_search_paper_publication_date_or_year(self):
        ...
        for date_range in date_ranges:
            with self.subTest(date_range=date_range):
                data = self.sch.search_paper(
                    "turing", publication_date_or_year=date_range, fields=["title"]
                )
    ...
    def test_search_paper_bulk_retrieval_sorted_results_default_order(self):
        data = self.sch.search_paper(
            "kubernetes", bulk=True, sort="citationCount", fields=["citationCount"]
        )
    ...
    def test_search_paper_bulk_retrieval_sorted_results_asc(self):
        data = self.sch.search_paper(
            "kubernetes", bulk=True, sort="citationCount:asc", fields=["citationCount"]
        )
        all_data = [item.citationCount for item in data]
        self.assertTrue(sorted(all_data) == all_data)
    ...
    def test_search_paper_bulk_retrieval_sorted_results_desc(self):
        data = self.sch.search_paper(
            "kubernetes", bulk=True, sort="citationCount:desc", fields=["citationCount"]
        )
        all_data = [item.citationCount for item in data]
    ...
    def test_search_paper_with_relevance_and_sort(self):
        with self.assertWarns(UserWarning):
            self.sch.search_paper("kubernetes", sort="citationCount", fields=["title"])
    ...
    def test_search_paper_match_title(self):
        ...
        paper = self.sch.search_paper(query, match_title=True, fields=["title"])
    ...
    def test_get_recommended_papers(self):
        data = self.sch.get_recommended_papers("10.2139/ssrn.2250500", fields=["title"])
    ...
    def test_get_recommended_papers_from_lists(self):
        data = self.sch.get_recommended_papers_from_lists(
            ["10.1145/3544585.3544600"], ["10.1145/301250.301271"], fields=["title"]
        )
        self.assertEqual(len(data), 100)
    ...
    def test_limit_value_exceeded(self):
        test_cases = [
            (
                self.sch.get_paper_authors,
                "10.1093/mind/lix.236.433",
                1001,
                "The limit parameter must be between 1 and 1000 inclusive.",
            ),
            (
                self.sch.get_paper_citations,
                "10.1093/mind/lix.236.433",
                1001,
                "The limit parameter must be between 1 and 1000 inclusive.",
            ),
            (
                self.sch.get_paper_references,
                "10.1093/mind/lix.236.433",
                1001,
                "The limit parameter must be between 1 and 1000 inclusive.",
            ),
            (
                self.sch.get_author_papers,
                1723755,
                1001,
                "The limit parameter must be between 1 and 1000 inclusive.",
            ),
            (
                self.sch.search_author,
                "turing",
                1001,
                "The limit parameter must be between 1 and 1000 inclusive.",
            ),
    ...
    def test_debug(self):
        ...
        with (
            self.assertLogs("semanticscholar", level="DEBUG") as log,
    ...
    def test_exception_internal_server_error(self, mock_request):
        mock_response = httpx.Response(status_code=500, json={"message": "message"})
    ...
    def test_exception_gateway_timeout(self, mock_request):
        mock_response = httpx.Response(status_code=504, json={"message": "message"})
        mock_request.return_value = mock_response
    ...
    def test_get_release(self):
        ...
        self.assertTrue(
            release.readme.startswith("Semantic Scholar Academic Graph Datasets")
        )
        ...
        self.assertTrue(
            first_dataset.description.startswith("Paper abstract text, where available")
        )
        self.assertTrue(
            first_dataset.readme.startswith("Semantic Scholar Academic Graph Datasets")
        )
    ...
    def test_get_dataset_download_links(self):
        ...
        self.assertTrue(
            dataset.description.startswith(
                "The core attributes of a paper (title, authors, date, etc.)"
            )
        )
        self.assertEqual(len(dataset.files), 30)
        self.assertTrue(
            dataset.files[0].startswith(
                "https://ai2-s2ag.s3.amazonaws.com/staging/2025-08-19/papers/20250822_0"
            )
        )
    ...
    def test_get_dataset_diffs(self):
        ...
        diffs = self.sch.get_dataset_diffs(
            dataset_name, start_release_id, end_release_id
        )
        ...
        self.assertEqual(
            diff.update_files[0],
            "https://ai2-s2ag.s3.amazonaws.com/updates/2024-10-08-to-2024-10-15/papers/20241018_1.gz",
        )
        self.assertEqual(
            diff.delete_files[0],
...
class AsyncSemanticScholarTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_paper_async(self):
        data = await self.sch.get_paper("10.1093/mind/lix.236.433", fields=["title"])
        self.assertEqual(data.title, "Computing Machinery and Intelligence")
        self.assertEqual(data.raw_data["title"], "Computing Machinery and Intelligence")
    ...
    async def test_get_papers_return_not_found_async(self):
        ...
        data = await self.sch.get_papers(list_of_paper_ids, return_not_found=True)
        papers = data[0]
        self.assertEqual(len(papers), 3)
        not_found = data[1]
        self.assertEqual(len(not_found), 1)
    ...
    async def test_get_author_papers_async(self):
        data = await self.sch.get_author_papers(1723755, limit=100, fields=["title"])
        self.assertEqual(data.offset, 0)
        self.assertEqual(data.next, 100)
    ...
    async def test_search_paper_year_async(self):
        data = await self.sch.search_paper("turing", year=1936, fields=["year"])
        self.assertEqual(data[0].year, 1936)

    @test_vcr.use_cassette
    async def test_search_paper_year_range_async(self):
        data = await self.sch.search_paper("turing", year="1936-1937", fields=["year"])
    ...
    async def test_search_paper_venue_async(self):
        data = await self.sch.search_paper("turing", venue=["ArXiv"], fields=["venue"])
        self.assertEqual(data[0].venue, "arXiv.org")
    ...
    async def test_search_paper_bulk_retrieval_async(self):
        data = await self.sch.search_paper("kubernetes", bulk=True, fields=["title"])
        self.assertEqual(data.total, 2674)
        self.assertEqual(len(data.items), 1000)
        self.assertEqual(
            data[0].title,
    ...
    async def test_search_paper_match_title_async(self):
        ...
        paper = await self.sch.search_paper(query, match_title=True, fields=["title"])
        self.assertEqual(paper.title, expected_title)
    ...
    async def test_get_recommended_papers_from_lists_negative_only_async(self):
        with self.assertRaises(BadQueryParametersException):
            await self.sch.get_recommended_papers_from_lists(
                [], ["10.1145/3544585.3544600"], fields=["title"]
            )
    ...
    async def test_get_dataset_diffs(self):
        ...
        start_release_id = "2024-10-08"
        end_release_id = "2025-08-19"
        diffs = await self.sch.get_dataset_diffs(
            dataset_name, start_release_id, end_release_id
        )
